...
/*
		...
		int position = -1;
		for(i = 0; i < 32; i++){
			if(pageTable[i].valid == TRUE && pageTable[i].virtualPage == vpn && pageTable[i].threadId == currentThread->get_tid_()){
				position = i;
				break;
			}
		}
		if(position != -1)
			entry = &pageTable[position];
		if(entry == NULL)
			return PageFaultException;
*/
	//	else
	//		printf("no page fault vpn %d, position %d, thread id %d.\n",entry->virtualPage,position,currentThread->get_tid_());
	//	printf("%d\n",position);
    } else {
  ...
  //  printf("tlbTotalHit %d, tlbTotalMiss %d, Miss rate %% %lf\n",tlbTotalHit,tlbTotalMiss,100.0*tlbTotalMiss/(tlbTotalHit+tlbTotalMiss));
